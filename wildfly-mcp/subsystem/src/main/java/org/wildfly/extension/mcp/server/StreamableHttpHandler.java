...
public class StreamableHttpHandler implements HttpHandler {
    ...
    private static final long DEFAULT_REQUEST_TIMEOUT_SECONDS = 300;
    ...
    public StreamableHttpHandler(ConnectionManager connectionManager, MCPMessageHandler handler,
        ...
        this(connectionManager, handler, sseHandler, Collections.emptySet(), DEFAULT_REQUEST_TIMEOUT_SECONDS);
    }

    public StreamableHttpHandler(ConnectionManager connectionManager, MCPMessageHandler handler,
            ServerSentEventHandler sseHandler, Set<String> allowedOrigins, long requestTimeoutSeconds) {
    ...
    public void handleRequest(HttpServerExchange exchange) throws Exception {
            ...
            if (sessionId != null) {
                ...
                if (existing == null) {
                    ...
                    sendJsonError(exchange, 404, null, JsonRPC.INVALID_REQUEST,
                            ROOT_LOGGER.unknownSessionError(sessionId));
                    return;
                }
        ...
        if (!isValidAcceptHeader(accepts)) {
            ROOT_LOGGER.invalidAcceptHeaders(accepts != null ? Arrays.toString(accepts.toArray()) : "");
        ...
        try {
            ...
            sendJsonError(exchange, 400, null, JsonRPC.INVALID_REQUEST, ROOT_LOGGER.invalidJson());
            return;
        }
        ...
        if (parsed.getValueType() == JsonValue.ValueType.ARRAY) {
            ...
            sendJsonError(exchange, 400, null, JsonRPC.INVALID_REQUEST,
                    ROOT_LOGGER.batchRequestNotSupported());
            return;
        }

        JsonObject content = asJsonObject(parsed);
        if (content == null) {
            sendJsonError(exchange, 400, null, JsonRPC.INVALID_REQUEST, ROOT_LOGGER.expectedJsonObject());
            return;
        }
        ROOT_LOGGER.debugf("Received message from client: %s", content);

        if (!validateMcpMethodHeader(exchange, content) || !validateMcpNameHeader(exchange, content) || !validateMcpParamHeaders(exchange, content)) {
            return;
        }
        ...
        if (protocolVersion != null && ProtocolVersion.from(protocolVersion).isEmpty()) {
            ...
            sendJsonError(exchange, 400, requestId, UNSUPPORTED_PROTOCOL_VERSION,
                    ROOT_LOGGER.unsupportedProtocolVersion(protocolVersion), data);
            return;
        }
        ...
        if (connection == null) {
            ...
            sendJsonError(exchange, 404, requestId, JsonRPC.INVALID_REQUEST,
                    ROOT_LOGGER.unknownSessionError(connectionId));
            return;
        }
        ...
        if (protocolVersion != null && connection.initializeRequest() != null) {
            ...
            if (negotiated != null && !negotiated.equals(protocolVersion)) {
                String requestId = extractRequestId(content);
                sendJsonError(exchange, 400, requestId, UNSUPPORTED_PROTOCOL_VERSION,
                        ROOT_LOGGER.protocolVersionMismatch(protocolVersion, negotiated));
                return;
            }
        }
        ...
        try {
            if (!responseLatch.await(requestTimeoutSeconds, TimeUnit.SECONDS)) {
                exchangeDone.set(true);
                sseConnection.cancel();
                try {
                    int id = sseConnection.lastEventId();
                    String errorJson = Messages.newError(extractRequestId(content),
                            JsonRPC.INTERNAL_ERROR, ROOT_LOGGER.requestTimedOut()).toString();
                    byte[] event = ("event: message\nid: " + id + "\ndata: " + errorJson + "\n\n")
                            .getBytes(StandardCharsets.UTF_8);
                    synchronized (os) {
                        os.write(event);
    ...
    private boolean validateMcpMethodHeader(HttpServerExchange exchange, JsonObject content) {
        ...
        if (mcpMethod == null) {
            ...
            if (ProtocolVersion.V_2026_07_28.is(protocolVersion)) {
                ...
                sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                        ROOT_LOGGER.missingMcpMethodHeader());
                return false;
            }
            return true;
        }
        if (!mcpMethod.equals(jsonRpcMethod)) {
            ...
            String requestId = extractRequestId(content);
            sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                    ROOT_LOGGER.mcpMethodHeaderMismatch(mcpMethod, jsonRpcMethod));
            return false;
        }
        return true;
    }
    ...
    private boolean validateMcpNameHeader(HttpServerExchange exchange, JsonObject content) {
        ...
        if (mcpName == null) {
            if (requiresName && !bodyName.isEmpty()) {
                ...
                if (ProtocolVersion.V_2026_07_28.is(protocolVersion)) {
                    String requestId = extractRequestId(content);
                    sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                            ROOT_LOGGER.missingMcpNameHeader(method));
                    return false;
                }
            }
            return true;
        ...
        if (!trimmedName.equals(bodyName)) {
            ...
            String requestId = extractRequestId(content);
            sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                    ROOT_LOGGER.mcpNameHeaderMismatch(mcpName, bodyName));
            return false;
        }
        return true;
    }
    ...
    private boolean validateMcpParamHeaders(HttpServerExchange exchange, JsonObject content) {
        ...
        for (var headerName : exchange.getRequestHeaders().getHeaderNames()) {
            ...
            if (arguments.containsKey(bodyKey)) {
                ...
                try {
                    ...
                    String requestId = extractRequestId(content);
                    sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                            ROOT_LOGGER.mcpParamInvalidBase64(paramKey));
                    return false;
                }
                if (!decodedHeader.equals(bodyValue)) {
                    ...
                    String requestId = extractRequestId(content);
                    sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                            ROOT_LOGGER.mcpParamHeaderMismatch(paramKey, headerValue, bodyValue));
                    return false;
                }
            }
        }
        for (Map.Entry<String, String> entry : headerMappings.entrySet()) {
            ...
            if (arguments.containsKey(bodyKey) && !presentParamHeaders.contains(headerKey)) {
                String requestId = extractRequestId(content);
                sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                        ROOT_LOGGER.missingMcpParamHeader(headerKey, bodyKey));
                return false;
            }
        }
        return true;
    ...
    // MCP 2026-07-28 "Value Encoding" — not RFC 2047; the spec defines its own =?base64?{data}?= sentinel.
    // See https://github.com/modelcontextprotocol/modelcontextprotocol/blob/main/docs/specification/2026-07-28/basic/transports/streamable-http.mdx#value-encoding
    ...
    private void handleStatelessRequest(HttpServerExchange exchange, JsonObject content) {
        if (Messages.isResponse(content)) {
            sendJsonError(exchange, 400, extractRequestId(content), JsonRPC.INVALID_REQUEST,
                    ROOT_LOGGER.responseEnvelopesNotAccepted());
            return;
        }
        ...
        if (is2026 && params != null) {
            ...
            if (meta != null) {
                ...
                if (bodyVersion != null && protocolVersionHeader != null
                    ...
                    sendJsonError(exchange, 400, requestId, HEADER_MISMATCH,
                            ROOT_LOGGER.protocolVersionMetaMismatch(protocolVersionHeader, bodyVersion));
                    return;
                }
            }
        }
        ...
        if (is2026 && metadata == null) {
            sendJsonError(exchange, 400, requestId, JsonRPC.INVALID_PARAMS,
                    ROOT_LOGGER.missingRequiredMeta(RequestMetadata.MCP_PROTOCOL_VERSION,
                            RequestMetadata.MCP_CLIENT_CAPABILITIES));
            return;
        }
        ...
        try {
            if (!latch.await(requestTimeoutSeconds, TimeUnit.SECONDS)) {
                ...
                sendJsonError(exchange, 504, extractRequestId(content),
                        JsonRPC.INTERNAL_ERROR, ROOT_LOGGER.requestTimedOut());
                return;
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            ...
            sendJsonError(exchange, 500, extractRequestId(content),
                    JsonRPC.INTERNAL_ERROR, ROOT_LOGGER.requestProcessingInterrupted());
            return;
        }
        ...
        synchronized (responses) {
            if (responses.isEmpty()) {
                sendJsonError(exchange, 500, extractRequestId(content),
                        JsonRPC.INTERNAL_ERROR, ROOT_LOGGER.noResponseGenerated());
    ...
    private void handleStatelessSubscriptionsListen(HttpServerExchange exchange, JsonObject content, RequestMetadata metadata) {
        ...
        try {
            if (!resultLatch.await(requestTimeoutSeconds, TimeUnit.SECONDS)) {
                return;
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
